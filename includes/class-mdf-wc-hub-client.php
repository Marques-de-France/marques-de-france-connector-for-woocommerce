 ...
 *   3. Safety-net: `mdf_wc_flush_unsynced_sales` AS recurring action (hourly) catches
...
class MDF_WC_Hub_Client {
	...
	public function __construct() {
		$this->hub_url  = MDF_WC_Settings::get_hub_url();
		$this->site_url = home_url();
		$this->token    = MDF_WC_Settings::get_secure_token();
	...
	private function register_action_scheduler_hooks() {
		// Retry single sale
		add_action( 'mdf_wc_retry_hub_sync', [ $this, 'as_retry_sale' ] );

		// Hourly flush of all unsynced sales
		add_action( 'mdf_wc_flush_unsynced_sales', [ $this, 'flush_unsynced_sales' ] );
	...
	// ---------------------------------------------------------------------------
	// Immediate sync (called from MDF_WC_Attribution after recording local sale)
	// ---------------------------------------------------------------------------
	...
	public function sync_sale( WC_Order $order ) {
		...
		$attribution = MDF_WC_Attribution::get_order_attribution( $order );
	...
	private function schedule_retry( $order_id ) {
		...
		if ( as_has_scheduled_action( 'mdf_wc_retry_hub_sync', [ (int) $order_id ], 'mdf-wc' ) ) {
			return;
		}

		as_enqueue_async_action( 'mdf_wc_retry_hub_sync', [ (int) $order_id ], 'mdf-wc' );
	}
	...
	private function mark_synced( string $order_id ) {
		...
		delete_transient( 'mdf_wc_unsynced_notice_count' );
	}
}
