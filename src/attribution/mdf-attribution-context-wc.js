 ...
 * SameSite=Lax), and stamps the WooCommerce session via AJAX so checkout can read them
 * without relying on either storage mechanism alone.
 *
 * Storage strategy (mirrors mdf-tracker.js used on Shopify storefronts):
...
( function () {
	...
	function init() {
		...
		if ( isAttributed && ! alreadyAttributed ) {
			...
			// Restore any cookies that were pruned (Safari ITP), then re-stamp the WC session.
